...
function fastExtractFacts(content: string, memoryId: number, userId: number, source?: string, category?: string): void {
  try {
    // Skip non-conversational content — regex patterns match too aggressively on code/infra text
    const skipSources = ["opencode@zanverse", "pi@verse", "claude", "system", "import", "sync"];
    const skipCategories = ["state", "task", "reference", "code", "infrastructure"];
    const src = (source || "").toLowerCase();
    const cat = (category || "").toLowerCase();
    if (skipSources.some(s => src.includes(s)) || skipCategories.includes(cat)) return;
    // Skip content that looks like code or config
    if (/[{}\[\]=>].*[{}\[\]=>]/.test(content) || /^\s*(import|export|function|const|let|var|class)\s/m.test(content)) return;
...
async function hybridSearch(
  ...
  if (sanitized) {
    ...
    } catch (ftsErr: any) { log.error({ msg: "fts_search_failed", error: ftsErr.message }); }
  }
  ...
  "insight": "one key non-obvious insight"
    ...
    if ((url.pathname === "/store" || url.pathname === "/memory" || url.pathname === "/memories") && method === "POST") {
      ...
      try {
        ...
        fastExtractFacts(content.trim(), result.id, auth.user_id, source, category);
